...
function GetNotionSyncStatus()
  awful.spawn.easy_async_with_shell("docker inspect notion-sync-cv | jq -r  '.[0].State.Status'", function(stdout)
    if trim(stdout) == "running" then
      notion_icon.image = gears.color.recolor_image(beautiful.notion.icon, beautiful.notion.color.active)
    else
      notion_icon.image = gears.color.recolor_image(beautiful.notion.icon, beautiful.notion.color.inactive)
    end
  end)
end
