...
local markup   = require("imports.util.markup")
...
local function factory(args)
    ...
    function cal.build(month, year)
        ...
        for x = 0, 6 do notifytable[#notifytable + 1] = os.date("%a",
            os.time { year = 2006, month = 1, day = x + cal.week_start }):sub(1, utf8.offset(1, 3)) .. " " end
        notifytable[#notifytable] = string.format("%s\n%s", notifytable[#notifytable]:sub(1, -2),
            string.rep(" ", st_day * 4))
        local strx
        for x = 1, mth_days do
            strx = x
            ...
            notifytable[#notifytable + 1] = string.format("%-4s%s", strx,
